...
/**
 * Hash the most recent commit's diff (HEAD~1..HEAD), excluding doc files.
 * Returns:
 *   - { status: "ok", hash, commitSha } when HEAD has a parent and there are non-doc changes
 *   - { status: "no_parent" } for initial commits (no HEAD~1)
 *   - { status: "no_changes" } when the diff is empty after doc-file exclusion
 *   - { status: "not_a_repo" } when not in a git repo
 */
export function computeCommitRangeHash() {
  try {
    execSync("git rev-parse --is-inside-work-tree", {
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"],
  ...
  let diff = "";
  try {
    const changedFiles = getNonDocChangedFiles("HEAD~1..HEAD");
    if (changedFiles.length > 0) {
      diff = execFileSync("git", ["diff", "HEAD~1..HEAD", "--", ...changedFiles], {
        encoding: "utf-8",
        stdio: ["pipe", "pipe", "pipe"],
      });
    }
