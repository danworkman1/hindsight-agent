 ...
 * (which git diff doesn't see). Returns null if not in a git repo.
 */
export function computeDiffHash() {
  let diff;
  try {
    diff = execSync("git diff HEAD", { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
  } catch {
  ...
  try {
    ...
    for (const path of untrackedList) {
      try {
        ...
        // Binary file or unreadable; include path only
  ...
  // Empty diff and no untracked files = nothing to review
...
function saveCache(cache) {
  // Ensure parent directory exists (defensive — should always be there)
  const dir = dirname(CACHE_PATH);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });

  // Check current file size before deciding whether to evict
  let currentSize = 0;
  if (existsSync(CACHE_PATH)) {
    try {
      currentSize = statSync(CACHE_PATH).size;
    } catch {
      currentSize = 0;
    }
  }
 ...
 */
export function setCachedReview(hash, entry) {
  if (!hash) return;
